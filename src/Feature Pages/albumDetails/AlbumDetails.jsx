...
const AlbumDetails = () => {
  ...
  const fetchAlbumById = async () => {
    setLoading(true);
    try {
      const { data } = await api.get(`${backendUrl}/api/albums/${albumId}`, {
  ...
  const downloadSelectedImages = async () => {
    ...
    const imagesToDownload = albumImages.filter((img) =>
      selectedImages.includes(img.imageId)
    );
    ...
    for (const image of imagesToDownload) {
      try {
        ...
        link.href = url;
        link.download = image.name || `image-${image.imageId}`;
  ...
  const downloadAllImages = async () => {
    ...
    toast.success(`Downloaded ${albumImages.length} image(s)`);
  };
                  ...
                  {filteredPhotos.map((image) => (
                    <div
                      ...
                      <div
                        ...
                        onClick={(e) => handleCheckmarkClick(e, image.imageId)}>
                        <BadgeCheck size={26} />
                      </div>
